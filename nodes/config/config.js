  ...
  function ConfigNode(n) {
    ...
    node.archiveSequences.forEach((sequence) => {
      ...
      const timer = (ms) => new Promise((resolve) => {
        setTimeout(() => {
          resolve(); // Resolve the promise when the timer is done, without returning a value.
        }, ms);
      });
      ...
      async function createNewArchive(srcPath, name) {
        try {
          const archiveSubPath = path.join(
            currentArchivePath,
            path.relative(path.resolve(logsPath), path.resolve(srcPath))
          );
          ...
          try {
            ...
            try {
              ...
              throw new Error(`[createNewArchive] Error creating directory${archiveSubPath}. Error: ${error}`);
            }
          }
          try {
       ...
       */
      async function dirCrawler(srcPath) {
        try {
          let filecounter = 0;
          ...
          if (filesToArchive.length) {
            ...
            do {
              ...
              try {
                ...
                throw new Error(`[dirCrawler] Creating archive failed: ${err}`);
              }
       ...
       */
      async function mainLoop(logPath) {
        const pendingDirs = [logPath];
        do {
          try {
