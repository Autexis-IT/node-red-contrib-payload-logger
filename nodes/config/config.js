...
// --> source: https://www.artpol-software.com/ziparchive/KB/0610051629.aspx
    ...
    node.archiveSequences.forEach(sequence => {
      ...
      try {
        ...
        cron.schedule(`${splitedTime[1]} ${splitedTime[0]} * * *`, function () {
          ...
          async function createNewArchive(srcPath, name) {
            try {
              ...
              return {zipper: tempZip, name: fullArchivePath};
          ...
          async function deleteFiles(files) {
            ...
            do {
              ...
              try {
                ...
                if (JSON.stringify(err).indexOf('EBUSY') >= 0) {
          ...
          async function dirCrawler(srcPath, fileArray = []) {
            try {
              ...
              const dirs = entries.filter(path => path.isDirectory()).map(dirent => dirent.name);
              let files = entries.filter(path => path.isFile()).map(dirent => dirent.name);
              ...
              for (let i = files.length - 1; i >= 0; i--) {
                ...
                try {
                  ...
                  if (!stat.size) {
                    ...
                    filesToArchive.push({filePath, time: stat.mtime})
                  }
                } catch (err) {
              ...
              if (filesToArchive.length) {
                ...
                do {
                  ...
                  updateStatus(currentFileAmount, 'adding files to archive');
                  offset += zipSize;
                  ...
                  // try to write zip
                  updateStatus(currentFileAmount, 'writing archive (eventloop blocked while writing)');
                  try {
                    let res = await zip.zipper.writeZipPromise(zip.name);
                  } catch (err) {
                  ...
                  // delete zipped files
                  updateStatus(currentFileAmount, 'deleting files');
              ...
              // log
              let message = (dirs.length ? `dir ${path.join(srcPath, dirs.toString())}` : `end of dir ${srcPath}`) + ` ${filesToArchive.length} files to process and ${filecounter} outdated files`;
              node.log(message);
                  ...
                  try {
                    if (dateToDelete !== undefined) {
                      if (archiveDate < dateToDelete) {
                        await fs.rm(path.resolve(path.join(archivePath, archiveName)), {force: true, recursive: true});
                      }
                    }
                  } catch (err) {
